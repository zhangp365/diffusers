...
from .pulid_encoders import IDFormer
...
class PulidProcessor(nn.Module):
    ...
    def load_pulid_state_dict(cls, pretrain_path=None, version='v0.9.1'):
        ...
        new_state_dict = {"pulid_encoder": {}, "pulid_ca": {}}
        for key in state_dict.keys():
            if key.startswith("pulid_encoder."):
                new_state_dict["pulid_encoder"][key.replace("pulid_encoder.", "")] = state_dict[key]
            elif key.startswith("pulid_ca."):
                data = state_dict[key]
                key = key.replace("pulid_ca.", "")
                if "to_kv." in key:
                    key_k = key.replace("to_kv.", "to_k.")
                    key_v = key.replace("to_kv.", "to_v.")
                    k, v = data.chunk(2, dim=0)
                    new_state_dict["pulid_ca"][key_k] = k
                    new_state_dict["pulid_ca"][key_v] = v
                    continue
                elif "norm1." in key:
                    key = key.replace("norm1.", "norm_cross.")
                elif "norm2." in key:
                    key = key.replace("norm2.", "group_norm.")
                elif "to_out." in key:
